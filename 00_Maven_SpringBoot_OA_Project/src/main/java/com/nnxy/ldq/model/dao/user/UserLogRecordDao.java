...
public interface UserLogRecordDao extends JpaRepository<LoginRecord, Long>{
		...
		//模糊查询  (from LoginRecord u where u.user.userId=?1 and ( DATE_format(u.loginTime,'%Y-%m-%d ') like %?2% or u.ipAddr like %?2%) order by u.loginTime DESC)
		...
		Page<LoginRecord> findByUserOrderBylogTimeDesc(Pageable pa);
